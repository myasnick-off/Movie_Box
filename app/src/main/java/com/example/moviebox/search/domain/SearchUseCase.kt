...
class SearchUseCase(private val remoteRepository: RemoteRepository) {
    ...
    suspend operator fun invoke(phrase: String, withAdult: Boolean, page: Int): Result<MovieListDTO> {
        return remoteRepository.searchByPhrase(phrase = phrase, withAdult = withAdult, page = page)
    }
}